...
def fill_missing_val():
    for column in df:
        df[column].fillna(-100, inplace = True) # Nahrazujeme vsechny chybejici hodnoty jako -100, df[column].min() pro nahrazeni NaN minimem
...
ct.fit(X_train) # Vypocte dulezite hodnoty pro transformaci a uloi je jako attributy transformatoru ct
